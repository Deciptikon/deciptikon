...
function formatIdSuffix(input) {
  return input
    .replace(/[.,\-–—]/g, "") // удаляем точки, запятые, дефисы и тире
    .replace(/\s+/g, "_") // пробелы → подчёркивание
    .replace(/_{2,}/g, "_") // несколько подчёркиваний подряд → одно
    .replace(/^_+|_+$/g, "") // обрезаем подчёркивания в начале и конце
    .toLowerCase();
}
...
/**
 * Подсчитывает количество символов в строке, игнорируя пробелы и переносы.
 * @param {string} str
 * @returns {number}
 */
